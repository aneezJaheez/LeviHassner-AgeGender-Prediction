...
flags.DEFINE_string('fold_dir', '/home/dpressel/dev/work/AgeGenderDeepLearning/Folds/train_val_txt_files_per_fold/test_fold_is_0',
                           'Fold directory')

flags.DEFINE_integer('val_fold', 0,
                           'Validation fold')

flags.DEFINE_string('data_dir', '/data/xdata/age-gender/aligned',
                           'Data directory')


flags.DEFINE_string('output_dir', '/home/dpressel/dev/work/AgeGenderDeepLearning/Folds/tf/test_fold_is_0',
                           'Output directory')


flags.DEFINE_string('train_list', 'age_train.txt',
                           'Training list')
flags.DEFINE_string('valid_list', 'age_val.txt',
                           'Test list')
...
def _find_image_files(list_file, data_dir):
    ...
    files_labels = [l.strip().split(' ') for l in tf.compat.v1.gfile.FastGFile(
...
def find_image_files(fold_dir, data_dir, mode="train", val_fold_number=1):
    labels_age = []
    labels_gender = []
    filenames = []

    for file in os.listdir(fold_dir):
        if mode == "train" and str(val_fold_number) in file:
            continue

        if mode == "val" and str(val_fold_number) not in file:
            continue
        ...
        for line in files_labels[1:]:
            jpeg_file_path = "%s/%s/%s/%s" % (data_dir, "aligned", line[0], "landmark_aligned_face." + line[2] + "." + line[1])
            if not os.path.exists(jpeg_file_path):
                continue
            if line[age_index] not in age_map.keys():
                continue
            if line[gender_index] not in gender_map.keys():
                continue
            filenames.append(jpeg_file_path)
            labels_gender.append(gender_map[line[gender_index]])
            labels_age.append(age_map[line[age_index]])
...
def _process_dataset(name, filename, directory, num_shards, mode, val_fold_number):
    ...
    """
    filenames, labels_age, labels_gender = find_image_files(filename, directory, mode=mode, val_fold_number=val_fold_number)
...
def main(unused_argv):
    ...
    #                  FLAGS.valid_shards)
    valid, valid_outcomes = _process_dataset("val", FLAGS.fold_dir, FLAGS.data_dir, FLAGS.valid_shards, mode="val", val_fold_number=FLAGS.val_fold)
    # train, train_outcomes = _process_dataset('train', '%s/%s' % (FLAGS.fold_dir, FLAGS.train_list), FLAGS.data_dir,
    #                  FLAGS.train_shards)
    train, train_outcomes = _process_dataset("train", FLAGS.fold_dir, FLAGS.data_dir, FLAGS.train_shards, mode="train", val_fold_number=FLAGS.val_fold)
